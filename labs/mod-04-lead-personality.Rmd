...
:::green
__Hint:__ Pay attention to the fact I wrote you want to prioritize the countries in the `combined` dataset. The `map_data("world")` dataset has way more countries than we need. So which join function do you want to use? Next, you need to specify in the join function which variables from the two datasets we should join on (using the `by = c(VAR1,VAR2)` argument). 
:::
