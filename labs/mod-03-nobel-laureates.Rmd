...
2. Create a new data frame called `nobel_living` that filters for: (a) laureates for whom `country` is available, (b) laureates who are people as opposed to organizations (organizations are denoted with `"org"` as their `gender`), (c) laureates who are still alive (their `died_date` is `NA`)
...
__Confirm that once you have filtered for these characteristics you are left with a data frame with `r nrow(nobel_living)` observations, once again using inline code.__ 
...
5. Add a second variable to your visualization from Exercise 3 based on whether the laureate was born in the US or not. Based on your visualization, do the data appear to support Buzzfeed's claim for this section? Explain your reasoning in 1-2 sentences.
